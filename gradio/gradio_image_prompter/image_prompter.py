...
class ImagePrompter(gradio.Image):
    ...
    def preprocess(self, x: PromptData) -> PromptValue | None:
        ...
        points = [point[:2] for point in x.points]
        coords = []
        grid_size = 30
        height = 480
        width = 640
        for point in points:
            coord = int(point[1]/height*grid_size) * grid_size + int(point[0]/width*grid_size)
            coords.append(coord)
        np.save('coord.npy',np.array(coords).astype(np.int32))
        return {"image": im, "points": points}
